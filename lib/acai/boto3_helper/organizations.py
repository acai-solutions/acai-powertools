...
class OrganizationsHelper:
    ...
    def __init__(self, logger: Loggable, boto_client: Any = None):
        """Initialize the OrganizationsHelper.

        Args:
            logger: Logger instance for debug/info/warning messages
            boto_client: Optional boto3 Organizations client. If not provided, creates one.
        """
        self.logger = logger
        self.boto_client = boto_client or boto3.client("organizations")
    ...
    def list_accounts(self, only_active: bool = True) -> dict[str, dict[str, Any]]:
        ...
        paginator = self.boto_client.get_paginator("list_accounts")
    ...
    def list_accounts_for_parent(self, parent_id: str) -> dict[str, dict[str, Any]]:
        ...
        result: dict[str, dict[str, Any]] = {}
        paginator = self.boto_client.get_paginator("list_accounts_for_parent")
    ...
    def list_child_ous(self, parent_ou_id: str, include_root: bool = True) -> list[str]:
        ...
        paginator = self.boto_client.get_paginator("list_children")
    ...
    def list_all_accounts_by_list(
        ...
        master_response = self.boto_client.describe_account(
    ...
    def get_ou_id(self, account_id: str) -> str | None:
        ...
        response = self.boto_client.list_parents(ChildId=account_id)
    ...
    def get_tags(self, account_id: str) -> Dict[str, str]:
        ...
        tags: Dict[str, str] = {}
        try:
            paginator = self.boto_client.get_paginator("list_tags_for_resource")
    ...
    def _get_organization_id(self) -> str | None:
        ...
        try:
            response = self.boto_client.describe_organization()
    ...
    def _get_organization_roots(self) -> dict[str, Any]:
        """Get all organization roots."""
        try:
            roots: dict[str, Any] = {}
            response = self.boto_client.list_roots()
    ...
    def _describe_account(self, account_id: str) -> dict[str, Any]:
        """Describe an account with error handling."""
        try:
            response = self.boto_client.describe_account(AccountId=account_id)
            return response
        except Exception as e:
    ...
    def _get_parent_ou(self, child_id: str) -> tuple[str | None, str]:
        ...
        if not child_id:
            self.logger.error("_get_parent_ou called with empty child_id")
            return None, ""
        ...
        try:
            parents = self.boto_client.list_parents(ChildId=child_id)
    ...
    def _get_ou_name(self, ou_id: str) -> str:
        ...
        try:
            response = self.boto_client.describe_organizational_unit(
    ...
    def _get_ou_id_with_path(self, account_id: str) -> tuple[str, str]:
        ...
        try:
            ...
            self.logger.error(
                f"Error getting OU ID with path for account {account_id}: {e}"
            )
        return "", ""
    ...
    def _get_ou_name_with_path(self, account_id: str) -> tuple[str, str]:
        ...
        try:
            ...
            self.logger.error(
                f"Error getting OU name with path for account {account_id}: {e}"
            )
        return "", ""
...
def organizations_list_accounts(
    boto_client: Any, logger: Loggable, only_active: bool = True
) -> dict[str, dict[str, Any]]:
    """List all accounts in the organisation, keyed by account ID."""
    helper = OrganizationsHelper(logger, boto_client)
...
def organizations_list_accounts_for_parent(
    boto_client: Any, logger: Loggable, parent_id: str
) -> dict[str, dict[str, Any]]:
    """List accounts directly under *parent_id*."""
    helper = OrganizationsHelper(logger, boto_client)
...
def organizations_list_child_ous(
    boto_client: Any, logger: Loggable, parent_ou_id: str, include_root: bool = True
) -> list[str]:
    """Recursively list all child OU IDs under *parent_ou_id*."""
    helper = OrganizationsHelper(logger, boto_client)
...
def organizations_get_ou_id(
    boto_client: Any, logger: Loggable, account_id: str
) -> str | None:
    """Return the parent OU ID for an account."""
    helper = OrganizationsHelper(logger, boto_client)
...
def organizations_get_member_account_context(
    boto_client: Any, logger: Loggable, member_account_id: str
) -> dict[str, Any] | None:
    """Get comprehensive context for an account."""
    helper = OrganizationsHelper(logger, boto_client)
