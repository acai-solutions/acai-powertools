...
class S3ObjectManager:
    ...
    def get_objects_from_bucket(
        ...
        result: list[dict[str, Union[str, bytes]]] = []
        ...
        for file in my_bucket.objects.filter(Prefix=prefix).all():
            if self.get_full_path(file.key) == prefix[:-1]:
                body: str | None = self.get_object_from_bucket(bucket_name, file.key)
                if body is not None:
                    entry: dict[str, Union[str, bytes]] = {
                        "objectKey": file.key,
                        "content": body.encode("utf-8"),  # Convert str to bytes
                    }
                    result.append(entry)
