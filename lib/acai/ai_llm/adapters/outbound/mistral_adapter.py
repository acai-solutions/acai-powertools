...
class MistralAdapter(LlmPort):
    ...
    # ------------------------------------------------------------------
    # Multi-turn tool calling (OpenAI-compatible wire format)
    # ------------------------------------------------------------------
    ...
    def chat_with_tools(
        ...
        *,
        ...
        """Single-turn tool-calling step (Mistral, OpenAI-compatible)."""
        try:
            self.logger.debug(
                "chat_with_tools",
                model=self.config.model_name,
            ...
            self.logger.error("Mistral chat_with_tools error", error=str(exc))
            raise ModelInvocationError(f"Mistral API error: {exc}") from exc

    def _call_tool_api_with_retries(
        self, kwargs: dict[str, Any]
    ) -> dict[str, Any]:
        """Call Mistral for a tool-using turn; return assistant-message dict."""
        import openai as _openai
        ...
        for attempt in range(self.config.max_retries + 1):
            try:
                ...
                if attempt < self.config.max_retries:
                    delay = self.config.retry_base_delay * (2**attempt)
                    self.logger.warning(
                        "Mistral rate limited (chat_with_tools), retrying",
                        attempt=attempt + 1,
                        delay=delay,
                    )
                    time.sleep(delay)
